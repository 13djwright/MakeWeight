    ...
    for (const { it, p } of parts) {
      ...
      tb.append(h('tr', { class: p.locked ? 'locked' : '', dataset: { key: 'p' + p.id } },
        ...
        h('td', null, h('span', { class: 'pill auto' }, (p.orient.mode || 'auto') + (p.orient.label ? ' · ' + p.orient.label : ''), p.locked ? ' 🔒' : ''), p.mirror ? h('span', { class: 'pill auto', style: { marginLeft: '4px' }, title: `Mirrored about ${(p.mirror_axis || 'x').toUpperCase()}` }, `⇋ ${(p.mirror_axis || 'x').toUpperCase()}`) : null,
          p.print && p.print.pair_mirror ? h('span', { class: 'pill auto', style: { marginLeft: '4px' }, title: `Mirrored pair: every other copy is the mirror image about the bed's ${String(p.print.pair_mirror).toUpperCase()} axis in the exported project` }, `pair ⇋${String(p.print.pair_mirror).toUpperCase()}`) : null,
    ...
    // printing options: supports (generated in the exported 3MF and sliced separately for the cost) and mirroring
    {
      ...
      right.append(h('div', { class: 'card' }, h('h3', null, 'Printing'),
        ...
        field('Mirrored', h('div', null, h('div', { class: 'seg' }, ...[['', 'No'], ['x', 'X'], ['y', 'Y'], ['z', 'Z']].map(([v, l]) => h('button', { 'aria-pressed': String((p.mirror_axis || '') === v), title: v ? `Reflect the part about the ${v.toUpperCase()} axis (in the part’s own frame, before orientation)` : 'As modelled', onClick: () => upd({ mirror_axis: v || null }) }, l))),
          h('p', { class: 'hint', style: { margin: '4px 0 0' } }, 'The whole part is sliced and exported flipped (all copies). For a left/right pair use “Mirrored pair” below instead; “Mirror copy” above makes the opposite-hand twin as a separate part.'))),
        field('Mirrored pair', h('div', null, h('div', { class: 'seg' }, ...[['', 'No'], ['x', 'about X'], ['y', 'about Y']].map(([v, l]) => h('button', { 'aria-pressed': String((pr.pair_mirror || '') === v), title: v ? `Every other copy is the mirror image about the bed’s ${v.toUpperCase()} axis` : 'All copies identical', onClick: () => upd({ print: { pair_mirror: v || null } }) }, l))),
          h('p', { class: 'hint', style: { margin: '4px 0 0' } }, `Left/right pairs on one sheet line: keep the quantity (${it.qty}) and every other copy in the exported project is the mirror image — same face on the bed, same weight, no second part needed.${it.qty < 2 ? ' (This line has quantity 1, so nothing is mirrored until it is 2 or more.)' : ''}`))),
